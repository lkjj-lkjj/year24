...
from serverA import pkgMapping
from serverA.Kernel_4_19 import *
from serverA.Kernel_5_10 import *
...
def form_vul_dict():
    ...
    for vul in vul_list:
        ...
        if vul['source'] == 'kernel-4.19' or vul['source'] == 'kernel-5.10':
        ...
        temp_list = re.split(r'\s+', vul["fixed_version"].strip())
        for temp in temp_list:
            source = get_source_from_version(temp)
            if source in vul_dict:
                vul_temp = copy.copy(vul)
                vul_temp['fixed_version'] = temp
                vul_dict[source].append(vul_temp)
            else:
                vul_dict[source] = []
                vul_temp = copy.copy(vul)
                vul_temp['fixed_version'] = temp
...
def get_source_from_version(version):
    pos = version.rfind('-')
    version = version[:pos]
    pos = version.rfind('-')
    source = version[:pos]
...
def version_compare_epoch_last(version1, version2):
    ...
    # compare revision
    pos1 = version1.revision.find('uelc')
    pos2 = version2.revision.find('uelc')
...
def hash_compare(vul_dict, os_list, kernel_version, kernel_arch, kernel_compare_version, pkg_source_map_dict, cmd):
    global LATEST_VERSION
    exist_vul = dict()
    table = PrettyTable(['漏洞 ID', '漏洞库包名', '目标主机版本', '已修复版本', '最新版本', '受影响二进制包名'])
    # detect pkgs vul
    os_len = len(os_list)
    num = 0
    for pkg in os_list:
                ...
                if if_pkg_version(vul_version):
                    ...
                    if version_compare_epoch_last(version1, version2) == 1:
                        if mistake_detect(pkg[0], version1) == 0:
                            if vul['cveid'] in exist_vul:
                                exist_vul[vul['cveid']]['affectPkg'] += ", " + pkg[0]
                                exist_vul[vul['cveid']]['local_version'] = pkg[1]
                                exist_vul[vul['cveid']]['latest_version'] = LATEST_VERSION
                            else:
                                temp = vul.copy()
                                temp["affectPkg"] = pkg[0]
                                temp['local_version'] = pkg[1]
                                temp['latest_version'] = LATEST_VERSION
                                exist_vul[vul['cveid']] = temp

        else:
            source = pkg_source_map_dict.get(pkg[0])
            if source:
                vul_list = vul_dict.get(source)
                if vul_list:
                    for vul in vul_list:
                        vul_version = vul["fixed_version"].strip()
                        if if_pkg_version(vul_version):
                            ...
                            version2 = get_parts_of_the_version(pkg[1])
                            if version_compare_epoch_last(version1, version2) == 1:
                                if mistake_detect(source, version1) == 0:
                                    if vul['cveid'] in exist_vul:
                                        exist_vul[vul['cveid']]['affectPkg'] += ", " + pkg[0]
                                        exist_vul[vul['cveid']]['local_version'] = pkg[1]
                                        exist_vul[vul['cveid']]['latest_version'] = LATEST_VERSION
    ...
    temp_list = list(exist_vul.values())
    for item in temp_list:
        if item['source'][:6] != 'kernel':
            table.add_row(
                [item['cveid'], item['source'], item['local_version'], item['fixed_version'], item['latest_version'],
                 item['affectPkg']])
    # print("\n\n软件包漏洞：")
    # print(table)
    # print("\n内核漏洞：")
...
def fix_vulnerability(exist_vul_list):
    if CLIENT:
        for item in exist_vul_list:
            # print('\n正在修复软件包：{}'.format(item['source']))
            try:
                pkgs = item['affectPkg'].split(',')
                for pkg in pkgs:
                    command = "echo {} | sudo -S yum update -y {}".format(PASSWORD, pkg)
                    stdin, stdout, stderr = CLIENT.exec_command(command, get_pty=True)

                item['is_fixed'] = True
                for pkg in pkgs:
                    if not is_fixed(item['fixed_version'], pkg):
                        item['is_fixed'] = False
...
def print_progress_bar(progress):
    ...
    progress_text = f"进程: [{'=' * block}{' ' * (bar_length - block)}] {int(progress * 100)}%"
...
def detect_run(os_list, kernel_version, kernel_arch, kernel_compare_version, client, server_type, cmd, system_version):
    ...
    try:
        ...
        print("ERROR: Can't exec [sudo yum makecache], please check your sudo permission; " + e.__str__())

    try:
        cmd.append_information("正在扫描...")
        exist_vul = hash_compare(vul_dict, os_list, kernel_version, kernel_arch, kernel_compare_version,
                                 pkg_source_map_dict, cmd)
        cmd.append_information("扫描结束！")
    except Exception as e:
        raise Exception("Scanning vulnerabilities error; " + e.__str__())

    exist_vul_list = list(exist_vul.values())
    ...
    if REPAIR:
        ...
        # print("漏洞修复中...")
        fix_vulnerability(exist_vul_list)
    ...
    if cmd.word or cmd.pdf or cmd.html or cmd.excel:
        ...
        if cmd.word:
            word_link = generate_doc(exist_vul_list, cmd.username, cmd.host,
                                     [current_time, current_time_end, str(round(consume_time, 2))], system_version,
                                     len(os_list),
                                     server_type)

        if cmd.pdf and not cmd.word:
            generate_doc(exist_vul_list, cmd.username, cmd.host,
                         [current_time, current_time_end, str(round(consume_time, 2))], system_version,
                         len(os_list),
                         server_type)
        ...
        if cmd.html:
            html_link = generate_html(exist_vul_list, cmd.username, cmd.host,
                                      [current_time, current_time_end, str(round(consume_time, 2))],
                                      system_version,
                                      len(os_list),
                                      server_type)

        if cmd.excel:
            excel_link = generate_excel(exist_vul_list, cmd, system_version,
    ...
    a, b, c, d, e = cal_lever(exist_vul_list)
