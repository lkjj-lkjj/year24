...
from desktop.Kernel_5_10 import *
from desktop.Kernel_4_19 import *
...
def get_parts_of_the_version(version):
    ...
    if res:
        ...
        return Version(epoch, upstream_version, debian_revision)
    else:
        return None
...
def is_fixed(vul_version, pkg_name):
    global CLIENT
    try:
        command = "apt policy " + pkg_name
        stdin, stdout, stderr = CLIENT.exec_command(command, get_pty=True)
        output = stdout.read().decode('utf-8', errors='replace')

        rule = r"已安装：(.+)"
        res = re.search(rule, output).group(1).strip('\n').strip('\r')
        ...
        for item in exist_vul:
            ...
            try:
                if item['source'] not in fixed_pkg:
                    # 发送命令
                    command = "echo {} | sudo -S apt -y install --only-upgrade {}".format(PASSWORD, item['source'])
                    stdin, stdout, stderr = CLIENT.exec_command(command, get_pty=True)
...
def version_compare_epoch_last(version1, version2):
    ...
    # compare the epoch
    v1e = '0' if version1.epoch == "" else version1.epoch
    v2e = '0' if version2.epoch == "" else version2.epoch
    if int(v1e) > int(v2e):
        return 1
...
def mistake_detect(package_name, vul_match_version):
    try:
        ...
        if package_name in PACKAGE_CACHE:
            ...
            _, result, _ = CLIENT.exec_command('apt-cache show {}'.format(package_name))
            result = result.read().decode()

            version_regex = r"Version:\s(.+)"
            match = re.search(version_regex, result)
            version_info = get_parts_of_the_version(match.group(1))
...
def hash_compare(vul_dict, os_list, kernel_version, kernel_arch, kernel_compare_version, cmd):
    exist_vul_pkg_name = set()
    exist_vul = []
    table = PrettyTable(['漏洞 ID', '漏洞库包名', '已修复版本', '最新版本', '本地包名', '目标主机版本'])
    # detect pkgs vul
    num = 0
    os_len = len(os_list)
    for pkg in os_list:
        ...
        if vul_list:
            for vul in vul_list:
                ...
                if if_version(vul_version) and if_version(pkg[1]):
                ...
                if version_compare_epoch_last(version1, version2) == 1:
                    if mistake_detect(vul["source"], version1) == 0:
                        table.add_row(
                            [vul['cveid'], vul['source'], vul['fixed_version'], LATEST_VERSION, pkg[0],
                             pkg[1]])
                        exist_vul_pkg_name.add(pkg[0])
    ...
    if kernel_vuls:
        for vul in kernel_vuls:
            vul_version = vul["fixed_version"].strip()
            if kernel_version == "4.19" and SERVER_TYPE == "Desktop":
                if if_kernel419_version(vul_version):
                    if kernel_4_19_version_compare(vul_version, kernel_compare_version, kernel_arch) == 1:
                        kernel_table.add_row(
                            [vul['cveid'], vul['source'], vul['fixed_version'], '.'.join(kernel_compare_version)])
                        # exist_vul.append(vul)
            if kernel_version == "4.19" and SERVER_TYPE == "ServerD":
                if if_kernel419_version_serverD(vul_version, vul['architecture'], kernel_arch):
                    if kernel_4_19_version_compare_serverD(vul_version, kernel_compare_version) == 1:
                        kernel_table.add_row(
                            [vul['cveid'], vul['source'], vul['fixed_version'], '.'.join(kernel_compare_version)])
                        # exist_vul.append(vul)
            if kernel_version == "5.10":
                if if_kernel510_version(vul_version):
                    if kernel_5_10_version_compare(vul_version, kernel_compare_version) == 1:
                        kernel_table.add_row(
                            [vul['cveid'], vul['source'], vul['fixed_version'], '.'.join(kernel_compare_version)])
                        # exist_vul.append(vul)
    # print("\n内核漏洞：")
    ...
    return exist_vul_pkg_name, exist_vul
...
def detect_run(os_list, kernel_version, kernel_arch, kernel_compare_version, client, server_type, cmd, system_version):
    ...
    global CLIENT, SERVER_TYPE, PASSWORD, REPAIR, NETWORK
    CLIENT = client
    ...
    try:
        if cmd.update:
            ...
            # print('正在更新源')
            if client:
                command = 'echo {} | sudo -S apt update'.format(PASSWORD)
                stdin, stdout, stderr = CLIENT.exec_command(command, get_pty=True)
                # print(stdout.read().decode('utf-8', errors='replace'))
            cmd.append_information("更新源成功！")

    except pexpect.ExceptionPexpect as e:
        print(f"本地列表更新失败，无法运行[sudo apt update]")
    except Exception as e:
        print("ERROR: Can't exec [sudo apt update], please check your sudo permission; " + e.__str__())

    try:
        cmd.append_information("正在扫描...")
        exist_vul_pkg_name, exist_vul = hash_compare(vul_dict, os_list, kernel_version, kernel_arch,
                                                     kernel_compare_version, cmd)
    ...
    for item in exist_vul:
        ...
        if not item['is_fixed']:
            if server_type == 'Desktop':
                item[
                    'fix_suggest'] = '可联网用户：通过apt命令升级软件包。内网用户：去SRC官网下载oup补丁包以及离线补丁升级工具进行更新，或通过在内网环境部署离线apt仓库源进行更新，其他情况请联系客户经理'
            else:
                item['fix_suggest'] = '可联网用户：通过apt命令升级软件包。内网用户：通过在内网环境部署离线apt仓库源进行更新，其他情况请联系客户经理'
        else:
            item['fix_suggest'] = ''
